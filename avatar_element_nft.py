...
while i < num:
    ...
    while len(random_list) < 3:
        random_int = np.random.randint(0,len(layers))
        if random_int in random_list:
            continue
        else:
            random_list.append(random_int)
    random_list = sorted(random_list)
    layers2 = [layers[x] for x in random_list]
    for element in layers2:
...
for params in params_dict2:
    j = 1
    frequency = params['Frequency']
    Class = params['Class']
    file_path = path + Class + '/' + params['对应素材编号'] + '.png'
    layer = Image.open(file_path).convert('RGBA')
    image_base64 = ''
    kiko_cat_info = {}
    for num in range(frequency):
        ...
        kiko_cat_info[Class] = params['对应素材编号'] + ' ## ' + str(j)
        kiko_cat_info['group_id'] = card_group_id
        kiko_cat_info['create_time'] = int(time.time() * 1000)
        kiko_cat_info['update_time'] = int(time.time() * 1000)
        kiko_cat_info['name'] = nft_name
...
composite_card_df = kiko_cat_info_df.reindex(columns = rename_dict.values())
